...
export async function adminRoutes(app) {
    ...
    app.get("/tenants/:id", {
        ...
        const [userCount, branchCount] = await tenantStorage.run(id, () => Promise.all([
    ...
    app.get("/tenants/:tenantId/users", {
        ...
        const users = await tenantStorage.run(tenantId, () => prisma.user.findMany({
    ...
    app.post("/impersonate/:tenantId", {
        schema: {
            tags: ["Admin"],
            summary: "Impersonar owner de un tenant",
            description: "Genera un access token de 1 hora que actúa como el owner del tenant. Útil para soporte técnico. No genera refresh token.",
            security: [{ bearerAuth: [] }],
            params: {
                type: "object",
                required: ["tenantId"],
        ...
        const ownerUser = await tenantStorage.run(tenantId, () => prisma.user.findFirst({
            where: {
                tenant_id: tenantId,
                is_active: true,
                role: { code: "owner" },
