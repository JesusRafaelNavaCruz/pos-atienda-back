...
// Helpers
function signTokens(app, payload) {
...
export async function authRoutes(app) {
    ...
    app.post("/register", {
        schema: {
            ...
            description: "Crea un nuevo tenant junto con su usuario owner. No requiere autenticación.",
            security: [],
            body: {
                type: "object",
                required: [
        ...
        const passwordHash = await bcrypt.hash(body.ownerPassword, env.BCRYPT_ROUNDS);
    ...
    app.post("/login", {
        schema: {
            ...
            description: "Autentica al usuario con email, contraseña y slug del tenant. Retorna access token y refresh token.",
            security: [],
            body: {
                type: "object",
        ...
        const user = await tenantStorage.run(tenant.id, () => prisma.user.findFirst({
            where: {
                tenant_id: tenant.id,
                email: body.email,
                is_active: true,
        ...
        const { accessToken, refreshToken, refreshPayload } = signTokens(app, jwtPayload);
        // Guardar sesión
        const tokenHash = await bcrypt.hash(refreshToken, 10);
        await tenantStorage.run(tenant.id, () => prisma.userSession.create({
            data: {
        ...
        await tenantStorage.run(tenant.id, () => prisma.user.update({
            where: { id: user.id },
    ...
    app.post("/refresh", {
        ...
        const session = await tenantStorage.run(decoded.tenantId, () => prisma.userSession.findFirst({
            where: {
                id: decoded.sub,
    ...
    app.post("/logout", {
        ...
        const { refreshToken } = refreshSchema.parse(req.body);
        const user = req.user;
        await tenantStorage.run(user.tenantId, () => prisma.userSession.deleteMany({
    ...
    app.post("/pin", {
        schema: {
            ...
            description: "Autentica a un usuario mediante su PIN de 4 dígitos. Retorna solo un access token de corta duración (8h).",
            security: [],
            body: {
                type: "object",
        ...
        const users = await tenantStorage.run(tenant.id, () => prisma.user.findMany({
            where: {
                tenant_id: tenant.id,
                is_active: true,
                pin_hash: { not: null },
    ...
    app.get("/profile", {
        schema: {
            ...
            description: "Retorna los datos del usuario autenticado, incluyendo su rol y permisos.",
            security: [{ bearerAuth: [] }],
            response: {
                200: {
        ...
        const user = await tenantStorage.run(jwtUser.tenantId, () => prisma.user.findUnique({
