...
export async function purchaseOrdersRoutes(app) {
    ...
    app.get('/suggest', {
        schema: {
            ...
            description: 'Retorna los productos con stock bajo (stock ≤ min_stock) del proveedor indicado, con la cantidad sugerida a pedir.',
            security: [{ bearerAuth: [] }],
            querystring: {
                type: 'object',
        ...
        const supplier = await tenantStorage.run(user.tenantId, () => prisma.supplier.findFirst({ where: { id: supplier_id, tenant_id: user.tenantId } }));
        if (!supplier) {
    ...
    app.get('/', {
        ...
        const [orders, total] = await tenantStorage.run(user.tenantId, () => Promise.all([
    ...
    app.get('/:id', {
        ...
        const user = req.user;
        const { id } = req.params;
        const order = await tenantStorage.run(user.tenantId, () => prisma.purchaseOrder.findFirst({
    ...
    app.post('/', {
        ...
        const order = await tenantStorage.run(user.tenantId, () => prisma.$transaction(async (tx) => {
            // Validar proveedor
            const supplier = await tx.supplier.findFirst({
                where: { id: body.supplier_id, tenant_id: user.tenantId, is_active: true },
            });
            if (!supplier)
                throw Object.assign(new Error('Proveedor no encontrado'), { code: 'NOT_FOUND' });
            ...
            if (products.length !== productIds.length) {
                ...
                throw Object.assign(new Error(`Productos no encontrados: ${missing.join(', ')}`), { code: 'NOT_FOUND' });
            }
    ...
    app.patch('/:id/send', {
        ...
        const user = req.user;
        const { id } = req.params;
        const existing = await tenantStorage.run(user.tenantId, () => prisma.purchaseOrder.findFirst({ where: { id, tenant_id: user.tenantId } }));
        if (!existing) {
        ...
        const order = await tenantStorage.run(user.tenantId, () => prisma.purchaseOrder.update({
            where: { id },
            data: { status: 'sent' },
            include: {
                supplier: true,
...
- Actualiza el costo (\`cost\`) del producto con el \`unit_cost\` del item`,
        ...
        const order = await tenantStorage.run(user.tenantId, () => prisma.$transaction(async (tx) => {
            ...
            if (!existing)
                throw Object.assign(new Error('Orden no encontrada'), { code: 'NOT_FOUND' });
    ...
    app.patch('/:id/cancel', {
        ...
        await tenantStorage.run(user.tenantId, () => prisma.purchaseOrder.update({ where: { id }, data: { status: 'canceled' } }));
        return res.send({
            success: true,
