...
export async function customerRoutes(app) {
    ...
    app.get("/", {
        schema: {
            ...
            description: "Retorna la lista paginada de clientes activos del tenant. Requiere el feature `customers` en el plan.",
            security: [{ bearerAuth: [] }],
            querystring: {
                type: "object",
                properties: {
        ...
        const [customers, total] = await tenantStorage.run(user.tenantId, () => Promise.all([
    ...
    app.get("/:id", {
        schema: {
            ...
            description: "Retorna el detalle del cliente incluyendo sus últimas 10 compras completadas y estadísticas de gasto total.",
            security: [{ bearerAuth: [] }],
            params: {
                type: "object",
                required: ["id"],
        ...
        const user = req.user;
        const { id } = req.params;
        const customer = await tenantStorage.run(user.tenantId, () => prisma.customer.findFirst({
        ...
        const stats = await tenantStorage.run(user.tenantId, () => prisma.sale.aggregate({
            where: {
                customer_id: id,
    ...
    app.post("/", {
        ...
        const customer = await tenantStorage.run(user.tenantId, () => prisma.customer.create({ data: { ...body, tenant_id: user.tenantId } }));
    ...
    app.put("/:id", {
        ...
        const existing = await tenantStorage.run(user.tenantId, () => prisma.customer.findFirst({ where: { id, tenant_id: user.tenantId } }));
        if (!existing) {
            return res.code(404).send({
                success: false,
        ...
        const customer = await tenantStorage.run(user.tenantId, () => prisma.customer.update({ where: { id }, data: body }));
        return res.send({
            success: true,
            data: customer,
        });
    ...
    app.delete("/:id", {
        schema: {
            ...
            description: "Realiza un soft delete del cliente (is_active = false).",
            security: [{ bearerAuth: [] }],
            params: {
                type: "object",
                required: ["id"],
        ...
        await tenantStorage.run(user.tenantId, () => prisma.customer.update({ where: { id }, data: { is_active: false } }));
        return res.send({
            success: true,
            data: {
    ...
    app.post('/:id/redeem', {
        schema: {
            ...
            description: "Descuenta puntos de lealtad del cliente. Cada punto equivale a $1 MXN de descuento.",
            security: [{ bearerAuth: [] }],
            params: {
                type: "object",
                required: ["id"],
        ...
        const customer = await tenantStorage.run(user.tenantId, () => prisma.customer.findFirst({ where: { id, tenant_id: user.tenantId } }));
        if (!customer) {
            return res.code(404).send({
                success: false,
                error: {
        ...
        const updated = await tenantStorage.run(user.tenantId, () => prisma.customer.update({
            where: { id },
            data: {
                loyalty_points: {
                    decrement: points
