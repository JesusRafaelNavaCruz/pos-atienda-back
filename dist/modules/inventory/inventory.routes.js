...
export async function inventoryRoutes(app) {
    ...
    app.get("/movements", {
        schema: {
            ...
            description: "Retorna el historial paginado de movimientos de inventario. Permite filtrar por producto, tipo y rango de fechas.",
            security: [{ bearerAuth: [] }],
            querystring: {
                type: "object",
                properties: {
        ...
        const user = req.user;
        const { page, limit, product_id, supplier_id, branch_id, type, from, to } = querySchema.parse(req.query);
        ...
        const [movements, total] = await tenantStorage.run(user.tenantId, () => Promise.all([
...
- \`loss\`: Merma o pérdida (disminuye stock)`,
            ...
            body: {
                ...
                properties: {
                    ...
                    delta: {
                        type: "number",
                        description: "purchase: positivo (entrada). loss: positivo, el sistema resta. adjustment: con signo (+/-).",
                    },
                    type: {
                        type: "string",
        ...
        const movement = await tenantStorage.run(user.tenantId, () => prisma.$transaction(async (tx) => {
            ...
            if (body.type === "purchase") {
                if (body.delta <= 0)
                    throw new Error("Una compra debe tener cantidad positiva");
                netDelta = body.delta;
            }
            else if (body.type === "loss") {
                if (body.delta <= 0)
                    throw new Error("Una merma debe tener cantidad positiva");
                netDelta = -body.delta;
            }
            else {
            ...
            if (after < 0)
                throw new Error(`Stock insuficiente: hay ${before} unidades, no se pueden restar ${Math.abs(netDelta)}`);
