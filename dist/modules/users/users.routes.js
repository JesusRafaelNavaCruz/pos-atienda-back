...
Los managers solo ven cajeros de su propia sucursal.`,
        ...
        const users = await tenantStorage.run(user.tenantId, () => prisma.user.findMany({
            where,
            select: {
    ...
    app.get('/roles', {
        ...
        const user = req.user;
        const roles = await tenantStorage.run(user.tenantId, () => prisma.role.findMany({
    ...
    app.put('/me/password', {
        ...
        const found = await tenantStorage.run(user.tenantId, () => prisma.user.findUnique({ where: { id: user.sub } }));
        if (!found)
        ...
        await tenantStorage.run(user.tenantId, () => prisma.user.update({ where: { id: user.sub }, data: { password_hash } }));
        // Invalidar todas las sesiones activas
        await tenantStorage.run(user.tenantId, () => prisma.userSession.deleteMany({ where: { user_id: user.sub } }));
        return res.send({
            success: true,
    ...
    app.get('/:id', {
        ...
        const user = req.user;
        const { id } = req.params;
        const found = await tenantStorage.run(user.tenantId, () => prisma.user.findFirst({
...
Los managers solo pueden crear usuarios con rol de cajero.`,
        ...
        const [sub, userCount] = await Promise.all([
            ...
            tenantStorage.run(user.tenantId, () => prisma.user.count({ where: { tenant_id: user.tenantId, is_active: true } })),
        ]);
        ...
        const existing = await tenantStorage.run(user.tenantId, () => prisma.user.findFirst({ where: { tenant_id: user.tenantId, email: body.email } }));
        if (existing) {
        ...
        const role = await tenantStorage.run(user.tenantId, () => prisma.role.findFirst({ where: { id: body.role_id, tenant_id: user.tenantId } }));
        if (!role) {
            return res.code(400).send({
                success: false,
        ...
        const newUser = await tenantStorage.run(user.tenantId, () => prisma.user.create({
            data: {
    ...
    app.put('/:id', {
        ...
        const existing = await tenantStorage.run(user.tenantId, () => prisma.user.findFirst({
            where: { id, tenant_id: user.tenantId },
            include: { role: true },
        }));
        if (!existing) {
        ...
        const updated = await tenantStorage.run(user.tenantId, () => prisma.user.update({
            where: { id },
            data: body,
            select: {
                id: true, email: true, full_name: true, is_active: true,
    ...
    app.delete('/:id', {
        ...
        await tenantStorage.run(user.tenantId, () => prisma.user.update({ where: { id }, data: { is_active: false } }));
    ...
    app.put('/:id/pin', {
        ...
        await tenantStorage.run(user.tenantId, () => prisma.user.update({ where: { id }, data: { pin_hash } }));
        return res.send({
            success: true,
