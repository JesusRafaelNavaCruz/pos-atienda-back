...
export function requireFeature(featureKey) {
    return async (req, res) => {
        ...
        if (!features) {
            ...
            features = Object.fromEntries(subscription.plan.features.map((f) => [f.feature_key, f.limit_value]));
...
export function requirePermission(resource, action) {
    return async (req, res) => {
        ...
        const hasPermission = role.permissions.some((rp) => rp.permission.resource === resource && rp.permission.action === action);
        if (!hasPermission) {
            return res.code(403).send({
                success: false,
                error: {
