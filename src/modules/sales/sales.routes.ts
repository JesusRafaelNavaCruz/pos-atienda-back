...
async function generateFolio(tenantId: string): Promise<string> {
  const last = await tenantStorage.run(tenantId, () =>
...
export async function salesRoutes(app: FastifyInstance) {
  ...
  app.get('/', {
    ...
    const [sales, total] = await tenantStorage.run(user.tenantId, () =>
      Promise.all([
  ...
  app.get('/:id', {
    ...
    const sale = await tenantStorage.run(user.tenantId, () =>
      prisma.sale.findFirst({
  ...
  app.post('/', {
    ...
    const sale = await tenantStorage.run(user.tenantId, () =>
      prisma.$transaction(async (tx) => {
        ...
        if (totalPaid < total) {
          throw new Error(
            `El total pagado (${totalPaid}) es menor al total de la venta (${total})`,
          )
        }
  ...
  app.post('/:id/cancel', {
    ...
    await tenantStorage.run(user.tenantId, () =>
      prisma.$transaction(async (tx) => {
        ...
        if (!sale) throw new Error('Venta no encontrada o ya cancelada')
  ...
  app.get('/summary/today', {
    ...
    const [sales, payments] = await tenantStorage.run(user.tenantId, () =>
      Promise.all([
